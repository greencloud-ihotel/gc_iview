<template>
  <ul class="super-search-list">
    <li
      v-for="(term, index) in conditions"
      :key="index"
    ...
    >
      ...
      >
        <span
          class="super-search-condition"
          :class="{ selected: term.selection.length }"
          >{{ term.title }}
        ...
        <div class="clearfix" slot="content">
          <div v-if="term.list" class="left super-search-select-box mr10">
            <ul class="super-search-select">
              <li
                ...
                :key="index"
                @click="setTableData(term, item, index)"
              >
                {{ `${item.descript}(${item.code})` }}
              </li>
            </ul>
          </div>
          <div class="right super-search-table">
            <Table
              ...
              :height="term.height"
              @on-selection-change="onSelectionChange($event, term, index)"
            ></Table>
          </div>
        </div>
      </Poptip>
  ...
  methods: {
    ...
    setTableData(term, item, index) {
      ...
      this.$http
        ...
        .then(res => {
          let data = term.dataPath
              ? this.getPath(res.data, term.dataPath)
              : res.data,
      ...
      handler() {
        ...
        this.conditions = this.columns.map(e => {
          ...
          !Array.isArray(each.columns)
            ? (each.columns = defaultCol)
            : each.columns.splice(0, 0, selection);
          return each;
        });
        // let data
        this.conditions.forEach((e, i) => {
          let codeMap = e.map
            ? Object.assign({}, this.codeMap, e.map)
            : this.codeMap;
          // handle left list
          e.listUrl &&
            this.$http
              .get(e.listUrl, { params: e.listParam })
              .then(res => {
                let data = !e.listPath
                  ? res.data
                  : this.getPath(res.data, e.listPath);
                e.list = data.map(de => {
                  return {
          ...
          e.autoGetData &&
            e.dataUrl &&
            this.$http
              .get(e.dataUrl, { params: e.dataParam })
              .then(res => {
                // console.log(res);
                let data = !e.dataPath
                  ? res.data
                  : this.getPath(res.data, e.dataPath);
                e.data = data.map(de => {
                  return {
